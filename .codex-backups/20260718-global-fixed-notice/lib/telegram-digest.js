...
export function studioFixedNoticeForEvent(state, event) {
  const eventType = cleanText(event.trainingType || event.type, 120);
  const rules = normalizeStudioTelegramSettings(state.studioTelegram || {}).rules
    .filter((rule) => rule.enabled !== false && rule.fixedNotice)
    .filter((rule) => rule.trainingType === "all" || rule.trainingType === eventType);
  const exactRule = rules.find((rule) => rule.trainingType === eventType);
  return cleanText((exactRule || rules[0])?.fixedNotice, 1500);
}
  ...
  try {
    ...
    if (action === "studio-preview" || action === "studio-send") {
      ...
      const message = buildStudioTelegramMessage(state, [studioEvent], {
        ...
        fixedNotice: studioFixedNoticeForEvent(state, studioEvent)
      });
